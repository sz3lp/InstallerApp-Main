...
const QuotesPage: React.FC = () => {
  const [
    quotes,
    {
      ...
      updateQuote,
      approveQuote,
      convertQuoteToJob: convertQuote,
      deleteQuote,
    },
  ] = useQuotes();
  ...
  const handleConvert = async (id: string) => {
    ...
    setConvertingId(id);
    try {
      const jobId = await convertQuote(id);
      setToast({ message: "Quote converted to job", success: true });
      setTimeout(() => setToast(null), 3000);
      if (jobId) navigate(`/jobs/${jobId}`);
    } catch (err: any) {
      setToast({ message: "Failed to convert quote: " + err.message, success: false });
      setTimeout(() => setToast(null), 3000);
    }
              ...
              <td className="p-2 border space-x-2">
                ...
                {q.status === "approved" && (
                  <SZButton
                    ...
                    disabled={convertingId !== null && convertingId !== q.id}
                  >
                    Convert to Job
                  </SZButton>
                )}
